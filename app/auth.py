from flask import Blueprint, render_template, request, redirect, url_for, session, url_for
...
def login():
    if request.method=="POST":
        codiceFiscale = request.form['Codice Fiscale']
        password = request.form['password']
        cursor.execute('SELECT * FROM Donatori WHERE coficeFiscale = %s AND password = %s',(codiceFiscale,password))
        record = cursor.fetchone()
        if record:
            session['loggedin'] = True
            session['codiceFiscale'] = record[1]
            return redirect(url_for('/dashboardDonatore'))
        else:
